...
export function ContactChannels({ locale }: { locale: Locale }) {
  ...
  const channels = [
    ...
    { name: locale === "ru" ? "Телефон" : locale === "hy" ? "Հեռախոս" : "Phone", detail: contacts.phoneDisplay, action: labels.phone, href: contacts.phoneHref, icon: "/social/phone.svg", kind: "phone" },
  ];
