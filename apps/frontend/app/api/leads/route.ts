...
const input = z.object({ phone: z.string().max(40).optional(), company: z.string().max(160).optional(), message: z.string().min(1).max(4000), source: z.string().max(80).default("site") });
...
export async function POST(request: Request) {
  ...
  const lead = await db.lead.create({ data: { ...parsed.data, phone: parsed.data.phone || null, company: parsed.data.company || null, userId: user.id, name: user.name, email: user.email } });
