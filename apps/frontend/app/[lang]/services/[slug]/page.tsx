...
const copy = {
  ...
  pl: { back: "Wszystkie usługi", format: "Format", budget: "Budżet początkowy", result: "Korzyść dla biznesu", roadmap: "Plan realizacji projektu", steps: ["Diagnoza", "Architektura", "Projektowanie i budowa", "Weryfikacja jakości", "Uruchomienie i rozwój"], stepText: ["Definiujemy cel, odbiorców, ograniczenia i mierniki sukcesu.", "Projektujemy ścieżkę użytkownika, zakres systemu i integracje.", "Tworzymy interfejs i działającą wersję w krótkich iteracjach.", "Testujemy kluczowe scenariusze, wydajność i responsywność.", "Uruchamiamy rozwiązanie, mierzymy efekt i przekazujemy jasny plan rozwoju."], action: "Dodaj usługę", calculate: "Wyceń projekt indywidualny", consult: "Umów bezpłatną konsultację", note: "Zakres i harmonogram potwierdzamy po krótkiej rozmowie. Każdy etap ma konkretny rezultat i punkt odbioru." },
...
function price(value: number, locale: Locale) {
  ...
  return new Intl.NumberFormat(locale === "ru" ? "ru-RU" : locale === "pl" ? "pl-PL" : "en-US", { style: "currency", currency: "RUB", maximumFractionDigits: 0 }).format(value);
}
...
export default async function ServiceDetail({ params }: { params: Promise<{ lang: string; slug: string }> }) {
  ...
  return <main className="service-detail shell">
    ...
    <section className="service-detail-hero">
      ...
      {service.tiers ? <div className="service-detail-tiers" aria-label={locale === "ru" ? "Цены по сложности проекта" : locale === "pl" ? "Ceny według złożoności projektu" : "Prices by project complexity"}>
